...
# affordance.dag = empty.graph(nodes = c("principal_component_0", "principal_component_1", "principal_component_2", "principal_component_3", "principal_component_4",
#                                        "action", "effect_X", "effect_Y"))
affordance.dag = empty.graph(nodes = c("principal_component_0", "principal_component_1",
                                       "action", "effect_X", "effect_Y"))
amat(affordance.dag) = matrix(c(0L, 0L, 0L, 1L, 1L,
                                0L, 0L, 0L, 1L, 1L,
                                0L, 0L, 0L, 1L, 1L,
                                0L, 0L, 0L, 0L, 0L,
                                0L, 0L, 0L, 0L, 0L),
                              byrow = TRUE, nrow = 5, ncol = 5, dimnames = list(nodes(affordance.dag), nodes(affordance.dag)))
# Plot the BN
...
nodeRenderInfo(pp) <- list(fontsize=c(20),
                          label=c("action"="Action",
                                  "effect_X"="Effect X",
                                  "effect_Y"="Effect Y",
                                  "principal_component_0"="PCA1",
                                  "principal_component_1"="PCA2"))
renderGraph(pp)

# Load train data
wd <- "/home/yiklungpang/catkin_ws/src/affordance_experiment/bn"
train_data_path <- file.path(wd, "processed_data", "pca_train_data.csv")
train_data <- read.csv(train_data_path, header = TRUE, sep = ",", quote = "\"", dec = ".", row.names = NULL)
# head(train_data)

# Compute CPT
bn.bayes <- bn.fit(affordance.dag, data = train_data, method = "bayes")

# Load test data
test_data_path <- file.path(wd, "processed_data", "pca_test_data.csv")
...
for (i in 1:nrow(test_data)) {
  ...
  evidence <- setEvidence(junction, nodes = c("principal_component_0",
                                              "principal_component_1",
                                              "action"), 
                         states = c(toString(test_data[i,1]),toString(test_data[i,2]),toString(test_data[i,3])))
  # Marginalize
  ...
  real_x <- toString(test_data[i,4])
  real_y <- toString(test_data[i,5])
  # Verify prediction
...
loo_data_path <- file.path(wd, "processed_data", "pca_loo_data.csv")
...
for (i in 1:nrow(loo_data)) {
  ...
  evidence <- setEvidence(junction, nodes = c("principal_component_0",
                                              "principal_component_1",
                                              "action"),
                          states = c(toString(loo_data[i,1]),toString(loo_data[i,2]),toString(loo_data[i,3])))
  # Marginalize
  effect.cpt <- querygrain(evidence, nodes = c("effect_X", "effect_Y"),
                           type = "joint")
  # Randomize if multiple max prob
  ...
  pred_x <- rownames(effect.cpt)[max_idx[1]]
  pred_y <- colnames(effect.cpt)[max_idx[2]]
  real_x <- toString(loo_data[i,4])
  real_y <- toString(loo_data[i,5])
  # Verify prediction
  if (pred_x == real_x & pred_y == real_y) {
